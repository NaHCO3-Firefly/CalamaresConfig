...
def mount_partition(root_mount_point, partition, partitions, mount_options, mount_options_list, efi_location):
    ...
    if fstype == "btrfs" and partition["mountPoint"] == '/':
        ...
        # Unmount after subvolume creation: some kernel versions
        # return EPERM/EISDIR after btrfs subvol create even when
        # the unmount succeeds. Use call() and verify instead.
        r = subprocess.call(["umount", "-v", root_mount_point])
        if r != 0:
            r = subprocess.call(["umount", "-l", root_mount_point])
        # Only fail if the mountpoint is actually still mounted
        if os.path.ismount(root_mount_point):
            raise OSError(f"Failed to unmount {root_mount_point}")
